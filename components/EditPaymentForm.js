import { View, Text, Pressable, FlatList, TextInput, TouchableOpacity } from "react-native"
...
const EditPaymentForm = () => {
	const { wallets, editPayment, selectedPaymentId, payments } = useStateContext()
	...
	const [paymentType, setPaymentType] = useState(payment.amount > 0)

	const selecteWalletHandler = (id) => (event) => setWalletId(id)
	...
	const editPaymentHandler = async () => {
		...
		await editPayment({ ...fields, amount: (paymentType ? 1 : -1) * fields.amount })
	}

	if (!payment || !wallets) return <View></View>
			...
			>
				<View
					style={{
						...
						borderRadius: 1000,
						marginRight: 5,
						backgroundColor: wallets.find((wallet) => walletId === wallet.id)?.color || "lightgray",
					}}
				/>
